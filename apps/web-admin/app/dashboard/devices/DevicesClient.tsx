...
import {
  addPrinter as addPrinterAction,
  setPrinterDefault as setPrinterDefaultAction,
  toggleActive as toggleActiveAction,
} from "./actions";
...
export default function DevicesClient() {
  ...
  const addPrinter = useMutation({
    mutationFn: async () => {
      ...
      const portNum = parseInt(pPort, 10);
      const res = await addPrinterAction({
        merchantId,
        label: pLabel,
        model: pModel,
        kind: pKind,
        ipAddress: pKind === "lan" ? pIp : null,
        port: pKind === "lan" ? (Number.isFinite(portNum) ? portNum : 9100) : null,
        cloudDeviceId: pKind === "cloud" ? pCloudId : null,
        isDefault: pDefault,
        supportsCashDrawer: pCashDrawer,
      });
  ...
  const setPrinterDefault = useMutation({
    mutationFn: async (id: string) => {
      if (!merchantId) throw new Error("No active merchant.");
      const res = await setPrinterDefaultAction({ merchantId, printerId: id });
      if (!res.ok) throw new Error(res.error);
    },
  ...
  const toggleActive = useMutation({
    mutationFn: async ({
      ...
      if (!merchantId) throw new Error("No active merchant.");
      const res = await toggleActiveAction({ merchantId, table, id, active });
      if (!res.ok) throw new Error(res.error);
    },
    onSuccess: (_d, vars) => {
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          {devicesQ.isLoading ? (
            ...
            <p className="p-6 text-sm text-red-600">
              {safeErrorMessage(devicesQ.error)}
            </p>
      ...
      <section className="space-y-3">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          {terminalsQ.isLoading ? (
            ...
            <p className="p-6 text-sm text-red-600">
              {safeErrorMessage(terminalsQ.error)}
            </p>
      ...
      <section className="space-y-3">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          {printersQ.isLoading ? (
            ...
            <p className="p-6 text-sm text-red-600">
              {safeErrorMessage(printersQ.error)}
            </p>
