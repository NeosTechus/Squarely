...
export async function POST(req: NextRequest) {
  ...
  // Per-user rate limit: Resend bills per send, so cap at 30/min/user. Placed
  // after auth so anon traffic 401s without consuming bucket entries, and
  // before the order fetch so a throttled caller does not hit Supabase.
  const rl = checkRateLimit(`receipts/email:${userId}`, 30, 60_000);
  if (!rl.allowed) {
    return NextResponse.json(
  ...
  if (!resp.ok) {
    ...
    console.error("[receipts/email] Resend error", resp.status, errBody);
    return NextResponse.json(
      { ok: false, error: safeErrorMessage(errBody, "Email send failed.") },
      { status: 502 },
    );
  }
