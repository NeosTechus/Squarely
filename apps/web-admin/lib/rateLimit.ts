...
//
// Callers should authenticate the request first and key by `${route}:${userId}`
// so unauthenticated traffic 401s before consuming any bucket entries and so
// different users get isolated buckets.
//
 ...
 *
 * @param key       Stable identifier for the bucket. Use `${route}:${userId}`.
